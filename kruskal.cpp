...
int disjointset_getroot(int u,std::vector<int> &parent)
{
  ...
  if(parent[u]!=u)parent[u]=disjointset_getroot(parent[u],parent);
...
{
  //Set the parent of root of one vertex to be the parent of the root of the other
    ...
    std::vector<std::vector<char>>& maze)
{
  ...
  for(int i=0;i<(int)Kruskalarray.size();i++)
  {
    ...
    if(disjointset_checksamecomponent(u,v,parent)) continue;
