...
int initmaze(int H, int W, std::vector<std::vector<char>> &maze) {
  ...
  // Allocate
  maze = std::vector<std::vector<char>>(2 * H + 1,
                                        std::vector<char>(2 * W + 2, 0));
...
void removeedge(int h1, int w1, int h2, int w2,
...
int plot(int H, int W, std::vector<std::vector<char>> &maze,
         const char *basename) {
  ...
  // If there is a horizontal barrier between adjacent cells in a column, draw
  // it
  for (int h = -1; h < H; h++)
    for (int w = 0; w < W; w++)
      if (maze[2 * h + 2][2 * w + 1] == '-')
